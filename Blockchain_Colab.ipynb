...
def home():
    return jsonify({
        'message': 'Blockchain API is running on Google Colab',
        'endpoints': {
...
print(f"\n🚀 Your Blockchain API is live at: {public_url}\n")
print("Available endpoints:")
print(f"  - {public_url} (Home)")
print(f"  - {public_url}/chain (View blockchain)")
print(f"  - {public_url}/mine (Mine a block)")
print(f"  - {public_url}/transaction/new (Create transaction)")
print(f"  - {public_url}/validate (Validate chain)")
print("\nPress Ctrl+C to stop the server")

threading.Thread(target=app.run, kwargs={'port': 5000, 'debug': False}).start()
